<template>
    <div class="detail-page">
        <div v-if="deposit">
            <h3>{{ deposit.bank_name }}</h3>
            <h1 style="margin-bottom: 30px;">{{ deposit.deposit_name }}</h1>
            ...
            <div>
                ...
                <div style="text-align: center;" v-if="selectedmonth">
                    <p style="color: rgb(156, 156, 156);">매달 <input type="text" v-model="money" class="money-input">원을 예금하면</p>
                    <p v-if="isNumeric(money)" style="font-size: 1.2em;">총 {{ (money * Number(maxoptions[selectedmonth]) * Number(month[selectedmonth]) / 12).toFixed(0) }}원의 이자를 받을 수 있어요!</p>
                    <p v-else>숫자를 입력해주세요!!</p>
            ...
            <div>
                ...
                <div style="margin: 15px 0px;">
                    ...
                    <p style="font-size: 1.3em;">{{ deposit.join_member }}</p>
                </div>
                <div style="margin: 15px 0px;">
                    <p style="margin: 0; color: rgb(156, 156, 156) ;">가입 방법</p>
                    <p style="font-size: 1.3em;">{{ deposit.join_way }}</p>
                </div>
                <div style="margin: 15px 0px;">
                    <p style="margin: 0; color: rgb(156, 156, 156) ;">상품 코드</p>
                    <p style="font-size: 1.3em;">{{ deposit.fin_prdt_cd }}</p>
                </div>
            </div>
        </div>
        <div v-else>
...
const isNumeric = function(money) {
    if (/^[0-9]+$/.test(money)) {
        return true
    } else {
        return false
...
onMounted(() => {
    ...
    axios({
        method: 'get',
        url: `${store.API_URL}/bankings/deposit-detail/${route.params.id}/`,
    }) .then((res) => {
        deposit.value = res.data
        basicoptions.value = {
            '6개월' : deposit.value.option6 !== '-' ? deposit.value.option6 : null,
            '12개월' : deposit.value.option12 !== '-' ? deposit.value.option12 : null,
            '24개월' : deposit.value.option24 !== '-' ? deposit.value.option24 : null,
            '36개월' : deposit.value.option36 !== '-' ? deposit.value.option36 : null,
        }
        maxoptions.value = {
            '6개월' : deposit.value.maxoption6 !== '-' ? deposit.value.maxoption6 : null,
            '12개월' : deposit.value.maxoption12 !== '-' ? deposit.value.maxoption12 : null,
            '24개월' : deposit.value.maxoption24 !== '-' ? deposit.value.maxoption24 : null,
            '36개월' : deposit.value.maxoption36 !== '-' ? deposit.value.maxoption36 : null, 
        }
